...
module.exports = function (argv, cmd) {
  if (cmd === 'mode') {
    if (![undefined, 'pwa', 'cordova', 'electron', 'test'].includes(argv.add)) {
      warn(`⚠️  Unknown mode "${ argv.add }" to add`)
      warn()
      process.exit(1)
    }
    if (![undefined, 'pwa', 'cordova', 'electron', 'test'].includes(argv.remove)) {
  ...
  if (!['spa', 'pwa', 'cordova', 'electron', 'test'].includes(argv.mode)) {
  ...
  if (cmd === 'build' && argv.mode === 'test') {
    // maybe later we could offer them the option to choose tests...
    // for now
    /*
  ...
  if (cmd === 'build' && argv.mode === 'test') {
    warn(`⚠️  Impossible to build in test mode`)
    warn()
    process.exit(1)
  }
  if (cmd === 'test' ) {

    // WIP check options here
  }
